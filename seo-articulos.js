...
const urlFacebook =
  "https://cvnet.cpd.ua.es/curriculum-breve/es/aparicio-arias-enrique-jesus/9322";
const urlInstagram = "";
const urlX = "";
const urlLinkedin =
...
function getPromptCategorias(titulo) {
  return `Classify this phrase "${titulo}" in one of these categories: ${categorias.toString()}. In the response, indicate only the category. The phrase and the categories are in ${language}`;
}
...
async function writeJsonToFile() {
  try {
    ...
    infoJson.authorsDescription = await chatgptMagic(
      promptAuthorsDescription()
    );
    infoJson.authorsSmallDescription = await chatgptMagic(
      promptAuthorsSmallDescription()
    );
    ...
    infoJson.categoryDescription = await chatgptMagic(
    ...
    infoJson.tiendaAmiga = [
      {
        description:
          "Siempre nos gusta colaborar con aquella tienda de cercanía, que tiene buen trato y unos productos excelentes. La Droguería de percon es nuestra opción.",
    ...
    infoJson.datosCuriososParaBorrar = await chatgptMagic(
      promptDatosCuriosos()
    );
    infoJson.datosCuriosos = [
      {
        title: "📺",
        description:
          "Los mandos a distancia son uno de los objetos más sucios de una casa. No suelen limpiarse a menudo, por no hablar de que todo el mundo los toca, ¡y a veces incluso con las manos sucias!",
        color: "bg-orange-900",
      },
      {
        title: "250 calorías",
        description:
          "La limpieza no solamente ayuda a mejorar el aspecto de nuestro hogar, ¡también quema calorías! ¿Sabías que puedes quemar hasta 100 calorías por hora con una limpieza ligera y hasta 250 con una limpieza más intensiva?",
        color: "bg-orange-900",
      },
      {
        title: "200,000",
        description:
          "Las bayetas de cocina pueden contener unas 200,000 bacterias. Eso es unas 200 veces más que una taza de baño. Por eso es importante lavarlas con regularidad.",
        color: "bg-orange-900",
      },
    ];
...
async function asignarCategoria(categoria) {
  if (
    categorias.find(
      (element) => slugify(element, { separator: "-" }) === categoria
    )
  ) {
    return categoria;
  }
    ...
    if (response.status === 200) {
      ...
      sharp(path)
        ...
        .toFile(pathWebp, (err, info) => {
          if (err) {
            console.error(
              "Error occurred while converting image to WebP:",
              err
            );
          } else {
            if (sufijo === "1") {
...
async function generarArticuloTraducido(
  keyword,
  imagenPrincipalUrl,
  imagenSecundariaUrl,
  categoria
...
async function addDate(contenido) {
  ...
  let stringToAdd = `\n\n_Artículo actualizado el ${localDate.toLocaleDateString(
    "es-ES",
    options
  )}_`;
...
`;
  ...
  if (videoPrincipalSEO !== undefined) {
    const newString =
      contenido.substring(0, indexPosition) +
      stringToAdd +
      contenido.substring(indexPosition);
    return newString;
  }
  return contenido;
}
...
async function createArticle() {
  ...
  let articulo = await chatgptMagic(
    promptArticulo(tituloSEO),
    "gpt-4-1106-preview"
  );
  descripcionSEO = await chatgptMagic(
    promptDescription(),
    "gpt-4-1106-preview"
  );
...
async function pictureOfTheDay() {
  ...
  const url =
    oldDate.length === 0
      ? `${baseApiUrl}/apod?api_key=${nasaApiKey}`
      : `${baseApiUrl}/apod?api_key=${nasaApiKey}&date=${oldDate}`;
  ...
  descripcionSEO = `Imagen astronómica del día ${localDate.toLocaleDateString(
    "es-ES",
    options
  )} por la NASA: ${tituloTraducido}`;
  descripcionSEO = await cleanTexto(descripcionSEO);
  ...
  cabeceroMarkdown += await getMetaData(
    tituloTraducido.replace(/[\n\r]+/g, ""),
    "https://" + dominio + "/" + categoriaSEO + "/" + urlSEO,
    dominio
  );
  cabeceroMarkdown += "\n---\n";

  cabeceroMarkdownpod += await getMetaData(
    tituloTraducidopod.replace(/[\n\r]+/g, ""),
    "https://" + dominio + "/" + categoriaSEO + "/" + urlSEOpod,
    dominio
  );
  cabeceroMarkdownpod += "\n---\n";
  ...
  if (data.media_type === "video") {
    ...
    articulo = await addPicture(
      articulo,
      `${tituloTraducido}${copyrightAutor}`
    );
  }
  ...
  if (data.media_type === "video") {
    articuloFinalpod = await addVideo(
      articuloFinalpod,
      `${tituloTraducido}${copyrightAutor}`
    );
    articuloFinalpod += `\n\n${stringToAdd}`;
    ...
    articuloFinalpod = await addPicture(
      articuloFinalpod,
      `${tituloTraducidopod}${copyrightAutor}`
    );
  }
...
async function traduccionTotal() {
  ...
  try {
    ...
    for (let file of files) {
      ...
      console.log(keyword);
      console.log("-----------------------------------------------------");
      const found = articulosTraducidos.find(
